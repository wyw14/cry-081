...
func (s *PublicationService) Search(ctx context.Context, filter PublicationFilter) (PublicationPage, error) {
	request := newPublicCatalogRequest(filter)
	page, err := s.articles.Search(ctx, request.repositoryFilter())
	if err != nil {
		return PublicationPage{}, err
	}
	return request.present(page), nil
}
...
func newPublicCatalogRequest(filter PublicationFilter) publicCatalogRequest {
	page := filter.Page
	if page < 1 {
		page = 1
	}
	...
	return publicCatalogRequest{query: filter.Query, sectionID: filter.SectionID, tag: filter.Tag, sort: sort, page: page, pageSize: pageSize}
}

func (r publicCatalogRequest) repositoryFilter() PublicationFilter {
	return PublicationFilter{Query: r.query, SectionID: r.sectionID, Tag: r.tag, Sort: r.sort, Page: r.page, PageSize: r.pageSize}
}

func (r publicCatalogRequest) present(page PublicationPage) PublicationPage {
	visible := make([]publication.Article, 0, len(page.Items))
	for _, article := range page.Items {
		if copy, ok := article.PublicCatalogCopy(); ok {

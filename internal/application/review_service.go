...
func (s *ReviewService) Assign(ctx context.Context, manuscriptID, reviewerID, editorID string, dueAt time.Time) (review.Assignment, error) {
	...
	plan, err := newReviewAssignmentPlan(target, reviewerID, editorID, dueAt, s.clock.UTCNow())
	if err != nil {
		return review.Assignment{}, err
	}
	assignment, err := plan.create(s.ids.NewID())
	if err != nil {
		return review.Assignment{}, err
	}
...
type reviewAssignmentPlan struct {
	manuscriptID      string
...
func newReviewAssignmentPlan(target manuscript.Manuscript, reviewerID, editorID string, dueAt, createdAt time.Time) (reviewAssignmentPlan, error) {
	plan := reviewAssignmentPlan{
		manuscriptID:      target.ID,
		manuscriptVersion: target.ActiveVersion,
	...
	if err := plan.validate(); err != nil {
		return reviewAssignmentPlan{}, err
	}
	return plan, nil
}

func (p reviewAssignmentPlan) validate() error {
	if p.manuscriptID == "" || p.manuscriptVersion < 1 {
		return shared.NewError("ASSIGNMENT_MANUSCRIPT_INVALID", "assignment manuscript is incomplete", shared.ErrValidation)
	}
	if p.reviewerID == "" || p.editorID == "" {
		return shared.NewError("ASSIGNMENT_ACTOR_INVALID", "assignment actors are incomplete", shared.ErrValidation)
	}
	if !p.dueAt.After(p.createdAt) {
		return shared.NewError("ASSIGNMENT_DUE_INVALID", "assignment due time must be in the future", shared.ErrValidation)
	}
	return nil
}

func (p reviewAssignmentPlan) create(id string) (*review.Assignment, error) {
	return review.NewAssignment(id, p.manuscriptID, p.manuscriptVersion, p.reviewerID, p.editorID, p.dueAt, p.createdAt)
}

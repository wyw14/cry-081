...
func (r Report) PublicView() Report {
	return reportProjectionFor(publicAudience).apply(r)
}

type reportAudience string

const (
	publicAudience    reportAudience = "public"
	editorialAudience reportAudience = "editorial"
)

type reportProjection struct {
	includeReviewer      bool
	includeAnonymousNote bool
	includeScores        bool
	includeComments      bool
	includeResponses     bool
}

func reportProjectionFor(audience reportAudience) reportProjection {
	policies := map[reportAudience]reportProjection{
		publicAudience: {
			includeReviewer:      false,
			includeAnonymousNote: true,
			includeScores:        true,
...
func (p reportProjection) apply(source Report) Report {
	projected := source.Clone()
	if !p.includeReviewer {
		projected.ReviewerID = ""
	}
	if !p.includeAnonymousNote {
		projected.AnonymousNote = ""
	}
	if !p.includeScores {
		projected.Scores = nil
	}
	if !p.includeComments {
		projected.Comments = nil
	}
	if !p.includeResponses {
		projected.Responses = nil
	}
	return projected
}

func (r Report) EditorialView() Report {
	return reportProjectionFor(editorialAudience).apply(r)
}

...
func (s *TokenStore) Revoke(ctx context.Context, request application.RefreshTokenRevocation) error {
	...
	s.identity.mu.Lock()
	defer s.identity.mu.Unlock()
	current, ok := s.identity.tokens[request.Token.ID]
	if !ok {
		return shared.ErrNotFound
	}
	if current.Version != request.ExpectedVersion || request.Token.Version <= request.ExpectedVersion {
		return shared.ErrVersionConflict
	}
	if request.Scope != application.RefreshTokenCurrentSession {
		return shared.NewError("TOKEN_REVOCATION_SCOPE_INVALID", "refresh token revocation scope is invalid", shared.ErrValidation)
	}
	return s.revokeUserSessions(request.Token)
}

func (s *TokenStore) revokeUserSessions(target identity.RefreshToken) error {
	if target.UserID == "" || target.RevokedAt == nil {
		return shared.NewError("TOKEN_REVOCATION_INVALID", "revoked token state is required", shared.ErrValidation)
	}
	when := target.RevokedAt.UTC()
	for id, stored := range s.identity.tokens {
		if stored.UserID != target.UserID || stored.RevokedAt != nil {
			continue
		}
		stored.RevokedAt = &when
		stored.Version++
		s.identity.tokens[id] = stored
	}
	return nil
}

...
type ReviewStore struct {
	...
	conflicts   []review.Conflict
}

func NewReviewStore() *ReviewStore {
	return &ReviewStore{assignments: map[string]review.Assignment{}, reports: map[string]review.Report{}, decisions: map[string]review.EditorialDecision{}}
}
...
func (s *ReviewStore) HasActiveConflict(ctx context.Context, editorID, authorID string, now time.Time) (bool, error) {
	...
	for _, conflict := range s.conflicts {
		if conflict.EditorID == editorID && conflict.AuthorID == authorID && conflict.ActiveAt(now) {
			return true, nil
		}
	}
	return false, nil
